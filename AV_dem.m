...
m_kTa = A_1*cos(w_c.*k*T_a + phi);  %Trägersignal der das Quellensignal zu hohen Frequenzen trägt m(t) = A*cos(w_c*t + phi)
X_kTa = (1+m*s_kTa).*m_kTa;    %moduliertes Signal x_(t) durch ein einfacher idealer modulator
Y_kTa = X_kTa;      %y_am(t) = x_am(t), weil n(t)=0. Keine additive Rauschen
Z_avd = abs(Y_kTa);     %z_avd(t) = |y_am(t)|
f_high = w_s;
f_low = -w_s;
...
m_kTa1 = A_1*cos(w_c1.*k*T_a1 + phi_1);  %Trägersignal der das Quellensignal zu hohen Frequenzen trägt m(t) = A*cos(w_c*t + phi)
...
X_kTa = (1+m*Sr_kTa).*m_kTa1;    %moduliertes Signal x_(t) durch ein einfacher idealer modulator
Y_kTa = X_kTa;      %y_am(t) = x_am(t), weil n(t)=0. Keine additive Rauschen
Z_avd = abs(Y_kTa);     %z_avd(t) = |y_am(t)|
f_high = w_sf;
f_low = -w_sf;
...
h_kTa = sinc(2*pi*12.5*k);     %Compute the lowpass filter h(kTa) with Grenzfrequenz 25Hz
